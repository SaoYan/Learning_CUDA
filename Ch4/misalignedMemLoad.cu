...
__global__ void sumArraysOnDevice(float *A, float *B, float *C, const int N, const int offset);
...
int main(int argc, char **argv) {
    ...
    start = clock();
    sumArraysOnDevice<<<grid, block>>>(d_A, d_B, d_C, nElem, offset);
...
__global__ void sumArraysOnDevice(float *A, float *B, float *C, const int N, const int offset) {
    ...
    // if (k < N) C[idx] = __ldg(&A[k]) + __ldg(&B[k]); // using read-only cache
}
